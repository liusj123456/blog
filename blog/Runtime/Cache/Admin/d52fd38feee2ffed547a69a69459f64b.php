					...
					<div class="table-responsive">
						...
						<table id="sample-table-2" class="table table-striped table-bordered table-hover">
							...
							<?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
									...
									<td><?php if($vo["status"] == 0): ?>可用<?php else: ?>不可用<?php endif; ?></td>
...
-->
											<button class="btn btn-xs btn-info" title="修改" onclick='javascript:location.href="<?php echo U(GROUP_NAME.'/User/userEdit',array('id'=>$vo['id']));?>";'>
												<i class="ace-icon fa fa-pencil bigger-120"></i>
											</button>

											<button class="btn btn-xs btn-danger" title="显示设置" onclick='javascript:if(confirm("确定更改吗？")) location.href="<?php echo U(GROUP_NAME.'/User/userDel',array('id'=>$vo['id']));?>";'>
