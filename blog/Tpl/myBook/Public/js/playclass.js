...
function PlayHistoryClass(){
	...
	this.viewPlayHistory=function (div){
		...
		/*if(checkcookie()){
		$.get("/user-Userdo-getplaylog", function(result){
			    if(result['rcode']>0){
					$("#his-todo").hide();
					if(result['r']!=null&&result['r'].length>0){
						...
						for(var i in result['r']){
							...
							var readurl="http://www.bookba.net/book-"+id+"-info.html";
							if(vodnames[1]!=undefined&&vodnames[1]!=null){
							innerStr += "<li><h5><a href=\"" + readurl + "\">"+vodnames[0]+"</a><em>/</em><a href='"+result['r'][i]['url']+"' target='_blank'>"+vodnames[1]+"</a></h5><label><a class=\"color\" href=\"" + result['r'][i]['url'] + "\">继续阅读</a></label><a href=\"javascript:;\" class='delck' data=\""+result['r'][i]['id']+"\" mtype='ajax'>删除</a></li>"
							}else{
							innerStr += "<li><h5><a href=\"" + result['r'][i]['url'] + "\">"+result['r'][i]['play_name']+"</a></h5><label><a class=\"color\" href=\"" + result['r'][i]['url'] + "\">继续阅读</a></label><a href=\"javascript:;\" class='delck' data=\""+result['r'][i]['id']+"\" mtype='ajax'>删除</a></li>"
							}
						}
		...
		if(navigator.cookieEnabled){
			...
			for(var i =nameArray.length - 1; i >= 0; i--){
				...
				var id=getidbyurl(urlArray[0]);
				var vodnames=nameArray[i].split('|');
				var readurl="http://www.bookba.net/book-"+id+"-info.html";
				//innerStr += "<li><a target=_blank class=name href=\"" + urlArray[i] + "\"" + " title=\"" + nameArray[i] + "\">" + texts + "</a><a target=_blank class=now href=\"" + urlArray[i] + "\"" + " title=\"" + nameArray[i] + "\">立即阅读</a></li>"
				if(vodnames[1]!=undefined&&vodnames[1]!=null){
				innerStr += "<li><h5>"+vodnames[0]+"<em>/</em><a href=\"" + urlArray[i] + "\">"+vodnames[1]+"</a></h5><label><a class=\"color\" href=\"" + urlArray[i] + "\">继续阅读</a></label><a href=\"javascript:;\" class='delck' data='"+i+"' mtype='inck'>删除</a></li>"
				}else{
				innerStr += "<li><h5>"+nameArray[i]+"</h5><label><a class=\"color\" href=\"" + urlArray[i] + "\">继续阅读</a></label><a href=\"javascript:;\" class='delck' data='"+i+"' mtype='inck'>删除</a></li>"
				}
			}
			}
			$("#his-todo").show();
	...
	this.addPlayHistory=function (name,url,cid,novelurl){
		if(checkcookie()){
...
suredo:function(e){
		if(type=='ajax'){
				$.get('/user-Userdo-emptyhistory',function(r){document.getElementById('playhistory').innerHTML="<li class='no-his'><p>暂无阅读历史列表...</p></li>";});
		}else{
			_GC();
		}
		$.closefloatdiv();
...
var id=new String(url.match(/read-(.*)-chapter/g)).replace('read-','').replace('-chapter','');
return parseInt(id);
}
