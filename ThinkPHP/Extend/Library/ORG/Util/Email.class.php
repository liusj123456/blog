...
class Email
{
	...
	public function __construct($user = '', $pass = '', $server = '', $port = 25, $timeout = 30, $auth = true, $charset = 'utf-8', $mail_type = 'html')
	{
		if(function_exists('C')){
			...
			$this->time_out 	= C('SMTP_TIME_OUT') != ''		? C('SMTP_TIME_OUT')		: $timeout;
			$this->auth 		= C('SMTP_AUTH') != ''			? C('SMTP_AUTH')			: $auth;
			$this->charset		= C('SMTP_MAIL_CHARSET') != ''	? C('SMTP_MAIL_CHARSET')	: $charset;
			$this->mail_type	= C('SMTP_MAIL_TYPE') != ''		? C('SMTP_MAIL_TYPE')		: $mail_type;
			$this->user_mail	= C('SMTP_USER_EMAIL') != ''	? C('SMTP_USER_EMAIL')		: $this->user;
		}else{
			$this->relay_host 	= $server;
			$this->smtp_port 	= $port;
			$this->user 		= $user;
			$this->pass 		= $pass;
